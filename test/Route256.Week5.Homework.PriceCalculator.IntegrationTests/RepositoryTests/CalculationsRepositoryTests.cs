...
public class CalculationsRepositoryTests
{
    ...
    public async Task Query_SingleCalculation_Success()
    {
        ...
        var foundCalculations = await _calculationRepository.Query(
            new CalculationHistoryQueryModel(userId, 1, 0, null), 
            default);
    ...
    public async Task Query_CalculationsInRange_Success(int take, int skip, int expectedCount)
    {
        ...
        var allCalculations = await _calculationRepository.Query(
            new CalculationHistoryQueryModel(userId, 100, 0, null), 
            default);
        ...
        // Act
        var foundCalculations = await _calculationRepository.Query(
            new CalculationHistoryQueryModel(userId, take, skip, null), 
            default);
    ...
    public async Task Query_Calculations_ReturnsEmpty_WhenForWrongUser()
    {
        ...
        // Act
        var foundCalculations = await _calculationRepository.Query(
            new CalculationHistoryQueryModel(anotherUserId, 100, 0, null), 
            default);
    ...
    [Fact]
    public async Task Query_DeleteFunc_Success()
    {
        // Arrange
        var userId = Create.RandomId();
        var now = DateTimeOffset.UtcNow;
        ...
        var foundGoods = await _calculationRepository.Delete(
            new DeleteHistoryQueryModel(userId, calculationIds.ToArray()),
            default);

        // Assert
        foundGoods.Should().NotBeEmpty();
        foundGoods.Should().OnlyContain(x => _calculationRepository.ConvertArrayOfArrayToSingleArray(calculations.Select(x => x.GoodIds)).ToArray().Contains(x));
    }

    [Fact]
    public async Task Query_AllCalculationsWithCalculationIds_Success()
    {
        // Arrange
        var userId = Create.RandomId();
        var now = DateTimeOffset.UtcNow;
        ...
        // Act
        var foundCalculations = await _calculationRepository.Query(
            new CalculationHistoryQueryModel(userId, 100, 0, calculationIds.ToArray()),
            default);
