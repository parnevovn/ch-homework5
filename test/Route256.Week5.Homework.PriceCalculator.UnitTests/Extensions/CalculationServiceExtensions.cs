...
public static class CalculationServiceExtensions
{
    ...
    public static Mock<ICalculationService> SetupDeleteCalculation(
        this Mock<ICalculationService> service)
    {
        service.Setup(p =>
                p.DeleteCalculations(
                    It.IsAny<DeleteCalculationFilter>(),
                    It.IsAny<CancellationToken>()));

        return service;
    }

    public static Mock<ICalculationService> VerifyDeleteCalculationWasCalledOnce(
        this Mock<ICalculationService> service,
        DeleteCalculationFilter filter)
    {
        service.Verify(p =>
                p.DeleteCalculations(
                    It.Is<DeleteCalculationFilter>(x => new DeleteCalculationFilterComparer().Equals(x, filter)),
                    It.IsAny<CancellationToken>()),
            Times.Once);
