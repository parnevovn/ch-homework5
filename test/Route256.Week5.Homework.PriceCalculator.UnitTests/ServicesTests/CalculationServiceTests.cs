...
public class CalculationServiceTests
{
    ...
    [Fact]
    public async Task QueryCalculations_WithCalculationIds_Success()
    {
        // arrange
        var userId = Create.RandomId();
        ...
        var queryModel = CalculationHistoryQueryModelFaker.Generate()
            ...
            .WithLimit(filter.Limit)
            .WithOffset(filter.Offset)
            .WithCalculationIds(filter.CalculationIds);
    ...
    [Fact]
    public async Task DeleteCalculations_Success()
    {
        // arrange
        const int goodsCount = 5;
        ...
        var filter = DeleteCalculationFilterFaker.Generate()
            .WithUserId(userId);

        var calculationsArray = calculations.Select(x => x.Id).ToArray();

        var builder = new CalculationServiceBuilder();

        builder.CalculationRepository
            .SetupDeleteCalculation(goodIds);
        ...
        //act
        await service.DeleteCalculations(new DeleteCalculationFilter(userId, calculationsArray), default);

        //asserts
        service.CalculationRepository
            .VerifyDeleteWasCalledOnce(new DeleteHistoryQueryModel(userId, calculationsArray));

        service.GoodsRepository
            .VerifyDeleteWasCalledOnce(userId, goodIds);

        service.VerifyNoOtherCalls();
    }

    [Fact]
    public async Task DeleteCalculations_ThrowsWhenOneOrManyCalculationsToAnotherUser()
    {
        // arrange
        const int goodsCount = 5;
        ...
        builder.CalculationRepository
            .SetupOneOrManyCalculationsToAnotherUserExeption(goodIds);

        var service = builder.Build();

        // Act, Assert
        await Assert.ThrowsAsync<OneOrManyCalculationsBelongsToAnotherUserException>(() => service.DeleteCalculations(new DeleteCalculationFilter(userId, calculationsArray), default));
    }

    [Fact]
    public async Task DeleteCalculations_ThrowsWhenOneOrManyCalculationsNotFound()
    {
        // arrange
        const int goodsCount = 5;
        ...
        var calculations = CalculationEntityV2Faker.Generate(5)
            .Select(x => x
                .WithUserId(userId)
                .WithPrice(calculationModel.Price)
                .WithTotalWeight(calculationModel.TotalWeight)
        ...
        builder.CalculationRepository
            .SetupOneOrManyCalculationsNotFoundException(goodIds);

        var service = builder.Build();

        // Act, Assert
        await Assert.ThrowsAsync<OneOrManyCalculationsNotFoundException>(() => service.DeleteCalculations(new DeleteCalculationFilter(userId, calculationsArray), default));
    }
}
